	...
	public static function getDivHTMLView($addon, &$addonList, $layouts, $pageName, $storeData = [], $storeListIndex = 0, $ids = [])
	{
		...
		if (!empty($addon->name))
		{
			if ($addon->name === 'div')
			{
				...
				$divElement .= '<div id="sppb-addon-' . $addon->id . '" class="sppb-div-addon ' . $addon->settings->class . $animation['class'] . ($global_custom_position && $global_select_position != '' ? ' sppb-positioned-addon ' : '') . '" ' . $animation['attributes'] . ($global_section_z_index ? 'data-zindex="' . $global_section_z_index . '"' : '') . ' ' . ($global_addon_z_index ? 'data-col-zindex="' . $global_addon_z_index . '"' : '') . ' ' . ($global_custom_position && $rowId ? 'data-rowid="' . $rowId . '"' : '') . ' ' . ($global_custom_position && $colId ? 'data-colid="' . $colId . '"' : '') . '>';
			}
			else
			{
